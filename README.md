...
1. [x] [5점] 무적시간동안 비행기가 반짝거리도록 한다.
1. [x] [10점] 남은 생명력을 막대기(5점)와 숫자(5점)로 표시한다.
1. [x] [10점] 총알을 여러 종류로 만들고, 종류별로 크기와 색깔을 다르게 표현한다.
1. [x] [5점] 총알의 종류마다 플레이어와 충돌했을 때 차감되는 생명력을 다르게 한다.
1. [x] [10점] 사용자의 가장 오래 버틴 생존 시간을 최대 10개까지 파일에 기록한다.
1. [x] [5점] 게임오버시에 기록된 생존시간을 화면에 출력한다.
1. [x] [5점] 게임오버시에 기록된 생존시간을 화면에 출력하며, 현재 기록이 순위권에 있을 경우 강조하여 표시한다.
